...
void Wheel::updateGeometry(Ref<RigidBody> body, float dt) {
    ...
    m_transform->setRotation(body->pose.q);
...
Car::Car(PhysicsHandler& phys): m_phys(phys) {
    ...
    auto lightDirection = uniform(vec3(0.5f, 0.0f, 2.0f));
    ...
    m_bodyShadow = ref<GameObject>("CarBodyShadow", std::vector<Ref<Component>>{
        ref<Mesh>(PlaneGeometry(1.0f, 2.0f, true), shadowMaterial)
    });

    m_bodyShadow->m_transform->setPosition({0, 1.0f, 0});
    m_bodyShadow->m_transform->setScale(2.2f);
    ...
    auto rb = m_body->getComponent<RigidBody>();

    rb->name = "CarBody";
    rb->canSleep = false;
    rb->staticFriction = 0.1f;
    rb->dynamicFriction = 0.005f;
    rb->restitution = 0.2f;
    rb->setBox(colliderSize, 210.0f);
    rb->setPosition({ 4.0f, 2.0f, -3.0f });
    ...
    for (size_t i = 0; i < 4; i++) {
        ...
        /* Wheel */
        auto wheel = ref<Wheel>();

        wheel->addComponent<Mesh>(
            car_wheel, 
...
void Car::update(
    ...
    m_forward = rb->pose.q * vec3(0, 0, 1.0f);
    m_velocity = glm::length(rb->vel);

    if (glm::dot(rb->vel, m_forward) < 0.0f)
    ...
    rb->applyForce(m_forward * -Fd, rb->localToWorld(vec3(0)));
    ...
    for (size_t i = 0; i < 4; i++) {
        ...
        vec3 hardpointW = rb->localToWorld(wheel->m_hardpoint);

        auto [exists, point, normal, dist] = phys.raycast(hardpointW, wheel->m_normal);

        // std::cout << "Wheel " << i << " raycast: " 
        //     << (exists ? "hit" : "miss") 
        //     << ", dist: " << dist 
    ...
    m_camPos = rb->localToWorld({ 0, 1.5f, -4.4f });
    m_camLookPos = rb->localToWorld({ 0, 1.2f, 0 });

    /* Body shadow */
    auto [exists, point, normal, dist] = phys.raycast(rb->pose.p, m_wheels[0]->m_normal);

    if (exists) {
        vec3 offsetPoint = rb->pose.p + (m_wheels[0]->m_normal * (dist - 0.02f));
